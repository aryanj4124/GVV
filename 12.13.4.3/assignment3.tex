...
\begin{align}
      T &= 6 - H \\
        &= \{6,5,4,3,2,1,0\}
\end{align}
Let X be a random variable which denotes the difference between the number of heads and number of tails,
\begin{align}  
      X &= H - T \\
        &= H - (6 - H) \\
        &= 2H - 6 \\
        &= \{-6,-4,-2,0,2,4,6\}
\end{align} 
Therefore, X can take values from the set \{-6,-4,-2,0,2,4,6\}.  
\newline
...
p_X(k) &= P(H - T = K) \\
       &= P(H = 3 + \frac{k}{2}) \\ 
       &= \frac{\comb{6}{3 + \frac{k}{2}}}{2^6}  \quad (\text{from} \eqref{eq:8})
\end{align}
\end{document}
